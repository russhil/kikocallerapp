...
export default function RecordingsScreen() {
  ...
  const processRecording = async (
    ...
    const updateStep = step => {
      ...
      setRecordings(prev =>
        prev.map(r =>
          r.path === recording.path ? { ...r, processingStep: step } : r,
        ),
      );
    };
    ...
    try {
      ...
      try {
        const callInfo = await RecordingMonitorModule.findCallInfoForTimestamp(
      ...
      if (isAutoProcess && callDirection && callDirection !== 'INCOMING') {
        await saveRecordingState(recording.path, {
          isProcessed: true,
          classification: 'SKIPPED_DIRECTION',
        });
        setRecordings(prev =>
          prev.map(r =>
      ...
      const callerDisplay =
        callName || callPhone || recording.filename || 'Unknown';
      try {
        await RecordingMonitorModule.showNotification(
          'Processing Call',
          `Analyzing audio from ${callerDisplay}...`,
          notifId,
        );
      } catch (e) {}
      ...
      try {
        ...
        formData.append('audio_file', {
          uri: recording.path.startsWith('file://')
            ? recording.path
            : `file://${recording.path}`,
          type: 'audio/mp4',
        ...
        if (res.ok) {
          ...
          if (data.transcript && data.transcript.trim().length > 0) {
            ...
            nativeLog(
              'Rec_Transcribe',
              `Multipart success: ${transcript.substring(0, 50)}...`,
            );
          }
          ...
          nativeError(
            'Rec_Transcribe',
            `Multipart failed: ${res.status} - ${errText}`,
          );
        }
      } catch (e) {
      ...
      if (!transcript) {
        try {
          nativeLog(
            'Rec_Transcribe',
            'Falling back to Base64 transcription...',
          );
          ...
          if (res.ok) {
            ...
            if (data.transcript && data.transcript.trim().length > 0) {
              ...
              nativeLog(
                'Rec_Transcribe',
                `Base64 success: ${transcript.substring(0, 50)}...`,
              );
            }
          } else {
            const errText = await res.text().catch(() => 'N/A');
            nativeError(
              'Rec_Transcribe',
              `Base64 failed: ${res.status} - ${errText}`,
            );
          }
        } catch (e) {
      ...
      if (orderJson) {
        ...
        try {
          ...
          const datePart = new Date()
            ...
            .replace(/-/g, '');
          let seq = parseInt(
            (await AsyncStorage.getItem(`seq_${datePart}`)) || '1',
            10,
          );
          ...
          try {
            ...
            const cleaned = base
              .replace(
                /^(Record|Call|Recording|CallRecording|call_recording|CALL|REC|record)[_\- ]*/i,
                '',
              )
          ...
          const newOrder = {
            orderId: `${
              shopName ? shopName.substring(0, 3).toUpperCase() : 'ORD'
            }-${nextNumId}`,
            recordingId: recording.path,
            customerName,
            customerPhone: callPhone || orderData.customer_phone || 'unknown',
            storeName: shopName,
          ...
          try {
            const recSyncObj = {
              ...
              durationMs: recording.duration
                ? Math.floor(recording.duration * 1000)
                : 0,
          ...
          try {
            await RecordingMonitorModule.showNotification(
              'Order Created!',
              `Order #${newOrder.orderId} from ${customerName}`,
              notifId,
            );
          } catch (e) {}
          showPopup(
      ...
      try {
        await RecordingMonitorModule.showNotification(
          'Processing Failed',
          `Failed to process ${recording.filename}`,
      ...
      setProcessingIds(prev => {
        const n = new Set(prev);
        n.delete(recording.path);
        return n;
      });
      setTimeout(() => {
        setProcessingSteps(prev => {
          const n = { ...prev };
          delete n[recording.path];
          return n;
        });
        setRecordings(prev =>
          prev.map(r =>
            r.path === recording.path ? { ...r, processingStep: null } : r,
          ),
        );
      }, 2000);
    }
      ...
      <View style={s.card}>
        <View style={s.cardRow}>
          ...
          >
            <Icon
              name={
                r.isProcessed ? (isOrder ? 'check' : 'minus') : 'microphone'
              }
              size={20}
              color={
                r.isProcessed
