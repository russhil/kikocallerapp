...
export default function EditOrderScreen() {
  ...
  const loadOrder = async () => {
    ...
    setTotalAmount(
      found.totalAmount ? String(Math.round(found.totalAmount)) : '0',
    );
  ...
  const saveOrder = async () => {
    ...
    showPopup(t('common.saved'), t('editOrder.updateSuccess'), 'check', [
      {
        text: 'OK',
        onPress: () => {
          hidePopup();
          trackOrderEdited(orderId);
          nav.goBack();
        },
      },
    ]);
  ...
  return (
    <SafeAreaView style={s.container} edges={['top']}>
      <View style={s.appBar}>
        ...
        <Text style={s.appBarTitle}>{t('editOrder.title')}</Text>
        <View style={{ flex: 1 }} />
        <TouchableOpacity
          ...
          hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }}
        >
          <Text style={s.saveHeaderText}>{t('common.save')}</Text>
        </TouchableOpacity>
      </View>
      ...
      >
        ...
        >
          <Field
            label={t('editOrder.customerName')}
            value={customerName}
            onChange={setCustomerName}
          />
          <Field
            label={t('editOrder.customerPhone')}
            value={customerPhone}
            onChange={setCustomerPhone}
            keyboard="phone-pad"
          />
          <Field
            label={t('editOrder.deliveryAddress')}
            value={address}
            onChange={setAddress}
            multiline
          />
          ...
          <View style={s.productsHeader}>
            <Text style={s.sectionTitle}>{t('common.products')}</Text>
            <TouchableOpacity
              style={s.addBtnWrap}
              onPress={addProduct}
              hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }}
            >
              <Text style={s.addBtnText}>{t('editOrder.addProduct')}</Text>
            </TouchableOpacity>
          </View>
          ...
          {products.map((p, i) => (
            <View key={i} style={s.productRow}>
              ...
              <View style={{ flex: 1, marginLeft: 10 }}>
                <TextInput
                  style={s.productInput}
                  value={p.name}
                  onChangeText={v => updateProduct(i, 'name', v)}
                  placeholder={t('editOrder.productNamePlaceholder')}
                  placeholderTextColor={Colors.textMuted}
                  maxFontSizeMultiplier={1.2}
                />
                <View style={{ flexDirection: 'row', marginTop: 6, gap: 8 }}>
                  <TextInput
                    style={[s.productInput, { flex: 1 }]}
                    value={p.quantity}
                    onChangeText={v => updateProduct(i, 'quantity', v)}
                    placeholder={t('editOrder.qtyPlaceholder')}
                    placeholderTextColor={Colors.textMuted}
                    keyboardType="numeric"
                    maxFontSizeMultiplier={1.2}
                  />
                  <TextInput
                    style={[s.productInput, { flex: 2 }]}
                    value={p.price}
                    onChangeText={v => updateProduct(i, 'price', v)}
                    placeholder={t('editOrder.pricePlaceholder')}
                    placeholderTextColor={Colors.textMuted}
                    keyboardType="numeric"
                    maxFontSizeMultiplier={1.2}
                  />
          ...
          <TouchableOpacity
            ...
            activeOpacity={0.7}
          >
            <Text style={s.saveBtnText}>{t('editOrder.saveChanges')}</Text>
          </TouchableOpacity>
        </ScrollView>
      </KeyboardAvoidingView>
    ...
    <View style={{ marginBottom: Spacing.lg }}>
      ...
      <TextInput
        style={[
          ...
          bold && { fontWeight: FontWeights.bold, fontSize: FontSizes.xl },
        ]}
        value={value}
