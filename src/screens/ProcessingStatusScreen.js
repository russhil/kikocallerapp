...
export default function ProcessingStatusScreen() {
  ...
  const loadData = async () => {
    try {
      ...
      const recList = Object.entries(recState).map(([path, data]) => ({
        path,
        ...data,
      }));
  ...
  const personalCount = recordings.filter(
    r => r.classification === 'PERSONAL_CALL',
  ).length;
  ...
  return (
    <SafeAreaView style={s.container} edges={['top']}>
      <View style={s.appBar}>
        ...
        <Text style={s.appBarTitle}>{t('processing.title')}</Text>
      </View>
      ...
      <FlatList
            ...
            <View style={s.statsCard}>
              <View style={s.statRow}>
                <View style={s.statBox}>
                  ...
                  <Text style={s.statLabel}>{t('processing.processed')}</Text>
                </View>
                ...
                <View style={s.statBox}>
                  ...
                  <Text style={s.statLabel}>{t('common.orders')}</Text>
                </View>
                <View style={s.statDivider} />
                <View style={s.statBox}>
                  <Text style={[s.statNum, { color: Colors.textMuted }]}>
                    {personalCount}
                  </Text>
                  <Text style={s.statLabel}>{t('processing.personal')}</Text>
                </View>
              </View>
            </View>

            {recentOrders.length > 0 && (
              <Text style={s.listTitle}>{t('processing.recentOrders')}</Text>
            )}
          </>
        )}
            ...
            <View style={s.orderRow}>
              ...
              <View style={{ flex: 1, marginLeft: 12 }}>
                ...
                <Text style={s.orderMeta}>
                  {order.customerName || t('processing.unknown')} ·{' '}
        ...
        ListEmptyComponent={() => (
          <View style={s.emptyState}>
            ...
            <Text style={s.emptyTitle}>{t('processing.emptyTitle')}</Text>
            <Text style={s.emptyDesc}>{t('processing.emptyDesc')}</Text>
          </View>
        )}
        refreshControl={
          <RefreshControl
