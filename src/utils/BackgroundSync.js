...
async function fetchWithRetry(url, options, retries = 3) {
  for (let i = 0; i < retries; i++) {
    try {
      ...
      nativeError(
        'BG_Fetch',
        `Fetch failed attempt ${i + 1}/${retries} for ${url} - Error: ${
          e.message
        }`,
      );
...
export async function syncBackgroundRecordings(taskData) {
  ...
  try {
    ...
    if (recordings.length === 0) {
      console.log(
        'No new (post-installation) recordings found during background scan.',
      );
      return;
    }
    ...
    console.log(
      '[BG] Uploading & Transcribing recording via MULTIPART:',
      recent.filename,
    );
    ...
    try {
      ...
      formData.append('audio_file', {
        uri: recent.path.startsWith('file://')
          ? recent.path
          : `file://${recent.path}`,
      ...
      transcribeRes = await fetchWithRetry(
        `${BASE_URL}/api/transcribe-gemini-multipart`,
        {
          method: 'POST',
      ...
      if (transcribeRes?.ok) {
        ...
        nativeError(
          'BG_Multipart',
          `Failed to transcribe. Status: ${transcribeRes?.status}, Response: ${tText}`,
        );
      }
    } catch (e) {
    ...
    if (!transcript) {
      console.log(
        '[BG] Falling back to Base64 encode via transcribe-raw-audio...',
      );
      try {
        const base64 = await RecordingMonitorModule.getFileBase64(recent.path);
        transcribeRes = await fetchWithRetry(
          `${BASE_URL}/api/transcribe-raw-audio`,
          {
            method: 'POST',
            headers: {
        ...
        if (transcribeRes?.ok) {
          ...
          if (data.transcript && data.transcript.trim().length > 0)
            ...
            nativeError(
              'BG_Base64',
              'Base64 transcription returned empty transcript.',
            );
        } else {
          const text = await transcribeRes?.text().catch(e => 'none');
          nativeError(
            'BG_Base64',
            `Base64 api request failed: ${transcribeRes?.status} msg: ${text}`,
          );
        }
      } catch (e) {
        nativeError(
          'BG_Base64',
          `Base64 transcription also failed exception: ${e.message}`,
        );
      }
    }
    ...
    if (!transcript) {
      nativeError(
        'BG_Transcribe',
        `Transcription produced no result. Aborting processing for: ${recent.filename}`,
      );
      try {
    ...
    try {
      ...
      if (res.ok) {
        ...
        nativeError(
          'BG_Extract',
          `extract-order network error status: ${res?.status} text: ${txt}`,
        );
      }
    } catch (e) {
      nativeError(
        'BG_Extract',
        `extract-order network error: ${e.message || e}`,
      );
    }
    ...
    if (orderJson) {
      ...
      let seq = parseInt(
        (await AsyncStorage.getItem(`seq_${dateStr}`)) || '1',
        10,
      );
